...
from screnning_agent.config import reserved_note_path, adult_symptom_path, symptom_folder, transformed_id_list_path
...
# 3
def main():
    full_emr_path = reserved_note_path
    symptom_path = adult_symptom_path
    transformed_id_list_file_path = transformed_id_list_path
    ...
    if os.path.exists(transformed_id_list_file_path):
        transformed_id_dict = dict()
        with open(transformed_id_list_file_path, 'r', encoding='utf-8-sig', newline='') as f:
...
def parse_symptom(data_dict, unified_id, symptom_dict, question_one, question_dict, results, index):
    ...
    result = call_open_ai(prompt, model_name='gpt_4_turbo')
